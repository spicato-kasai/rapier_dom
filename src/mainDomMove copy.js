...
async function initWorld() {
	...
	vertsArr = paths.map((path, i) => {
		const rawVerts = pathToVertices(path, 2, 20);
	...
	bodies = [
		...
		world.createRigidBody(
			RAPIER.RigidBodyDesc.dynamic()
				.setTranslation(toPhysX(centers[2].x - 100), toPhysY(centers[2].y))
				.setLinearDamping(linearDamping)
				.setAngularDamping(angularDamping),
		),
		world.createRigidBody(
			RAPIER.RigidBodyDesc.dynamic()
				.setTranslation(toPhysX(centers[3].x + 100), toPhysY(centers[3].y))
				.setLinearDamping(linearDamping)
				.setAngularDamping(angularDamping),
		),
	];
	...
	for (let i = 0; i < 2; i++) {
		...
		const poly = cleaned.map(([x, y]) => [x, y]);
		decomp.makeCCW(poly);
		const convexParts = decomp.quickDecomp(poly);

		for (const part of convexParts) {
			if (part.length < 3) continue;
			const desc = RAPIER.ColliderDesc.convexHull(new Float32Array(part.flat()));
			if (!desc) continue;
			desc.setDensity(3);
			desc.setRestitution(0.0);
			const col = world.createCollider(desc, bodies[i + 2]);
			// 最初のパーツをcharacterController用に保持
			if (!colliders[i + 2]) colliders[i + 2] = col;
		}
	}
...
(async () => {
	...
	window.addEventListener("mousemove", (e) => {
		...
		for (let i = 0; i < bodies.length; i++) {
			...
			characterController.computeColliderMovement(colliders[i], delta, RAPIER.QueryFilterFlags.EXCLUDE_SENSORS, undefined, (collider) => collider.handle !== colliders[i].handle);
